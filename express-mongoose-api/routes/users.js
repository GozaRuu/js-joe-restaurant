...
/* GET users listing. */
router.get('/', cors.corsWithOptions, authenticate, verifyAdminRights, (req, res) => {
	Users.find({})
		.then((users) => {
			res.statusCode = 200;
...
router.post('/login', cors.corsWithOptions, passport.authenticate('local', { failureRedirect: 'users/login' }), (req, res) => {
	const token = getToken({ _id: req.user._id }); // create jwt and send it on login
	res.statusCode = 200;
	res.setHeader('Content-Type', 'application/json');
	res.json({ success: true, token, status: 'Login Successful' });
